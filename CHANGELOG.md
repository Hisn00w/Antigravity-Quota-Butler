...
- **新增额度重置提醒功能**：当模型额度从低于 90% 恢复到 90% 以上时，自动发送通知提醒用户额度已重置。
- **仪表盘新增设置项**：在偏好设置中新增"额度重置提醒"开关，默认关闭，样式与其他设置卡片统一。
- **状态栏显示优化**：将默认状态栏显示从 "AGQ" 改为 "Quota"，更加直观易懂。
- **多语言支持**：为额度重置提醒功能添加了完整的 5 语言支持（中文、英文、日文、法文、德文）。
- **配置优化**：新增 `ag-quota.enableResetNotification` 配置项，用户可在仪表盘或设置中灵活控制。
- **文档完善**：README 新增快速开始部分，包含状态栏使用说明和截图（支持 5 种语言）。
